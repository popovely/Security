	...
	<c:choose>
		<c:when test="${ not empty user_id }">
			...
			<!-- 
				...
				위와 같으므로 로그아웃의 URL은 아래와 같이 설정한다.
			 -->
		...
		<c:otherwise>
			<!-- 
				...
				설정하지 않았다면 default값이 "/login"이므로 아래와같이 기술한다.
			 -->
			<!-- 
			 	만약 해당 설정값을 추가했다면
			 	컨텍스트 루트 경로를 제외한 나머지 경로를 통해 action속성을 기술해야 한다.
			 -->
	...
	<%-- 	<form:form name="loginFrm" action="${ loginUrl }" method="post"> --%>
			...
			<form:form name="loginFrm" action="../myLoginAction" method="post">
				<!-- 
					...
					위와같이 기술하였으므로 로그인에 실패한 경우에는 아래와같이 처리하면 된다.
				 -->
				...
				<!-- 
					...
					이와같이 설정되어 있으므로 name속성은 아래와같이 기술한다.
				 -->
				<p>
